...
def draw(): #making rectangles
    ...
    screen.draw.textbox(question[0].strip(), questions_box,
        color="white", shadow=(0.5,0.5),
        scolor="dim grey"
    )
...
def update():
    move_marquee()       #moving marqueebox
...
def reading():
    ...
    q_file.close()                      #it opens question file.

def read_next_question():
    global question_index
    question_index=question_index+1
    return questions.pop(0).split(",")     #it will give you next question.
...
def on_mouse_down(pos):
    ...
    if skip_box.collidepoint(pos):
        skip_question()                    #it will give you a sign if you click the answer_boxes.
...
def skip_question():
    ...
    if questions and not is_game_over: #if you got more question but the game is not over
        question=read_next_question()
        time_left=30
    else:
        game_over()
...
def update_time_left():
    global time_left
    if time_left: #if there is more time then time has go down
